...
function renderPrimary(vm = {}) {
  ...
  return `<div class="workflow-focus"><section class="workflow-primary" role="region" aria-labelledby="workflow-primary-title"><h2 id="workflow-primary-title">本轮概览</h2><dl class="workflow-primary-grid">${primaryField("当前阶段", overview.currentPhase, false, "phase")}${primaryField("整体进度", overview.overallProgress, false, "progress")}${primaryField("采集进度", overview.acquisitionProgress, false, "acquisition")}${primaryField("完整 JD", overview.jdProgress, false, "jd")}${overview.site === 'zhaopin' ? primaryField("已分析", overview.analysisProgress, false, "analysis") : primaryField("可用推荐", overview.usableRecommendations, false, "recommendations")}${primaryField("剩余工作", overview.remainingWork, false, "remaining")}${overview.site === 'zhaopin' && overview.completed ? '' : primaryField("预计继续时间", overview.estimatedContinuation, false, "eta") + primaryField("暂停/阻塞原因", blockerDetail, true, "blocker")}${overview.site === "zhaopin" && overview.completed ? "" : primaryField("下一步", overview.nextAction, false, "next-action")}</dl></section></div>`;
}
...
function renderRunMetrics(header = {}) { if (header.site === 'zhaopin') return ''; return `<section class="workflow-run-metrics"><h2>本轮统计</h2><dl><div><dt>本轮目标</dt><dd>${number(header.targetSuccessCount)}</dd></div><div><dt>本轮成功</dt><dd>${number(header.successfulCount)}</dd></div><div><dt>今日进度</dt><dd>${number(header.todaySuccessful)} / ${number(header.dailyTarget)}</dd></div></dl></section>`; }

function renderProgressTracks(progress = {}) {
  if (!progress?.visible) return "";
  const order = progress.site === "zhaopin" ? ["scan", "jd", "analysis"] : ["scan", "jd", "analysis", "communication"];
...
function renderPhase(phase = {}) {
  ...
  if (phase.kind === "completed" && phase.site === "zhaopin") return `<section class="workflow-phase"><div class="workflow-done"><h2>智联本轮已完成</h2><p>完整 JD 和分析已保存，可查看只读结果。</p></div><div class="workflow-actions"><a class="button-link" data-workflow-primary="true" href="${escapeAttr(phase.queueHref)}">查看智联结果</a><a class="button-link" href="${escapeAttr(phase.planHref)}">返回今日任务</a></div></section>`;
  ...
  if (phase.kind === "interrupted") {
    if (phase.communication?.error) return `<section class="workflow-phase">${renderCommunicationStop(phase.communication.error)}<div class="workflow-actions"><a class="button-link" data-workflow-primary="true" href="${escapeAttr(phase.communicationHref)}">${escapeHtml(phase.communication.detailsLabel)}</a></div></section>`;
    const issue = phase.error || {};
    const impact = phase.communicationHref ? communicationErrorLabel(phase.errorCode) : issue.impact;
    return `<section class="workflow-phase"><div class="workflow-actions">${phase.communicationHref ? `<a class="button-link" data-workflow-primary="true" href="${escapeAttr(phase.communicationHref)}">${escapeHtml(phase.communication?.detailsLabel || "检查沟通中断项")}</a>` : renderResume(phase.resume, phase.site)}</div><div class="workflow-alert"><strong>${escapeHtml(issue.title || "本轮已中断")}</strong><p>${escapeHtml(impact || "当前进度已保留。")}</p><p>${escapeHtml(issue.nextAction || "返回本轮查看状态。")}</p></div><details class="workflow-error-technical"><summary>技术信息</summary><p>错误编号：${escapeHtml(phase.errorCode)}</p>${phase.errorMessage ? `<p>${escapeHtml(phase.errorMessage)}</p>` : ""}</details></section>`;
  }
...
function renderConfirmed(phase) { const communication = phase.communication || {}; return `<section class="workflow-phase"><h2>${communication.error ? "沟通已中断" : "清单已确认"}</h2><p>已选择 ${number(communication.summary?.total)} 个岗位，本轮成功目标 ${number(phase.targetSuccessCount)}。</p>${renderCommunicationStop(communication.error)}${communication.runtimeBlock ? `<div class="workflow-alert">${escapeHtml(communication.runtimeBlock)}</div>` : ""}<div class="workflow-actions">${communication.executionEnabled ? `<form method="post" action="/api/communication-control"><input type="hidden" name="batchId" value="${escapeAttr(communication.batchId)}"><button data-workflow-primary="true" name="action" value="${escapeAttr(communication.action)}">${escapeHtml(communication.actionLabel)}</button></form>` : ""}<a class="button-link"${communication.executionEnabled ? "" : ' data-workflow-primary="true"'} href="${escapeAttr(communication.detailsHref)}">${escapeHtml(communication.detailsLabel || "检查清单详情")}</a></div></section>`; }
function renderCommunicating(phase) { const communication = phase.communication || {}; const rows = Object.entries(communication.summary?.statusCounts || {}).map(([status, count]) => `<tr><th>${escapeHtml(communicationStatusLabel(status))}</th><td>${number(count)}</td></tr>`).join(""); return `<section class="workflow-phase"><h2>${communication.error ? "沟通已中断" : "正在沟通"}</h2>${renderCommunicationStop(communication.error)}${communication.runtimeBlock ? `<div class="workflow-alert">${escapeHtml(communication.runtimeBlock)}</div>` : ""}<table class="workflow-status-table"><tbody>${rows}</tbody></table><div class="workflow-actions"><a class="button-link" data-workflow-primary="true" href="${escapeAttr(communication.detailsHref)}">查看执行明细</a></div></section>`; }
function renderResume(resume, site) { if (!resume) return ""; const identity = `<input type="hidden" name="workflowRunId" value="${escapeAttr(resume.runId)}">`; if (resume.fixedAuthority) return `<form method="post" action="${escapeAttr(resume.endpoint)}">${identity}${resume.cdpPort ? `<input type="hidden" name="cdpPort" value="${number(resume.cdpPort)}">` : ""}<input type="hidden" name="browserMode" value="${escapeAttr(resume.browserMode)}"><span class="hint">${escapeHtml(site === "zhaopin" ? "创建本轮时固定的智联搜索页；浏览器：" + (resume.browserMode === "portable" ? "OfferGo 专用 Edge（推荐）" : "使用当前 Edge（高级，需要浏览器连接组件）") : resume.browserMode === "portable" ? "创建本轮时固定的 BOSS 搜索页；浏览器：OfferGo 专用 Edge（推荐）" : "创建本轮时固定的 BOSS 标签页；浏览器：使用当前 Edge（高级，需要浏览器连接组件）")}</span><button data-workflow-primary="true">继续本轮</button></form>`; return `<form method="post" action="${escapeAttr(resume.endpoint)}">${identity}<select name="browserMode"><option value="edge"${resume.browserMode === "edge" ? " selected" : ""}>使用当前 Edge（高级，需要浏览器连接组件）</option><option value="portable"${resume.browserMode === "portable" ? " selected" : ""}>OfferGo 专用 Edge（推荐）</option></select><button data-workflow-primary="true">继续本轮</button></form>`; }
