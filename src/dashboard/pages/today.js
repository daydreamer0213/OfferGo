...
function renderTodayPage(vm) {
  ...
  ${page.site === 'zhaopin' ? '' : `<div class="today-overview">
...
function renderPlatformSelector(vm) {
  ...
  const platformControl = enabled.length > 1
    ? `<form method="get" action="/plan"><input type="hidden" name="planId" value="${escapeAttr(page.planId)}"><label>本次找岗平台 <select name="site" aria-label="本次找岗平台" onchange="this.form.submit()">${enabled.includes('boss') ? `<option value="boss"${zl ? '' : ' selected'}>BOSS</option>` : ''}${enabled.includes('zhaopin') ? `<option value="zhaopin"${zl ? ' selected' : ''}>智联</option>` : ''}</select></label><noscript><button>切换平台</button></noscript></form>`
    : `<span class="workflow-budget">本次找岗平台：${zl ? '智联' : 'BOSS'}</span><a class="button quiet" href="/settings/platforms">管理平台</a>`;
  const dualControl = enabled.includes('boss') && enabled.includes('zhaopin')
    ? `<form class="workflow-start dual-platform-start" method="post" action="/api/workflow-run"><input type="hidden" name="planId" value="${escapeAttr(page.planId)}"><input type="hidden" name="browserMode" value="${vm.runtime?.browserMode === 'portable' ? 'portable' : 'edge'}">${vm.runtime?.browserMode === 'portable' ? '<input type="hidden" name="cdpPort" value="9222">' : ''}<button class="secondary" name="site" value="both">同时运行 BOSS + 智联</button></form>`
    : '';
  return `<div id="platform-search-actions" class="button-row">${platformControl}${dualControl}${zl ? `<button class="secondary" type="button" data-platform-action="open">准备智联搜索页</button><button class="secondary" type="button" data-platform-action="save">保存智联条件</button><p role="status" aria-live="polite" data-platform-feedback>${escapeHtml(vm.form?.acquisition?.inheritedPreview?.summary || '')}</p><script>(()=>{const buttons=[...document.querySelectorAll('[data-platform-action]')];const note=document.querySelector('[data-platform-feedback]');buttons.forEach(button=>button.addEventListener('click',async()=>{buttons.forEach(b=>b.disabled=true);note.textContent=button.dataset.platformAction==='save'?'正在读取并保存智联条件…':'正在准备智联搜索页…';try{const response=await fetch('/api/platform-search/'+button.dataset.platformAction,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({site:'zhaopin',planId:${Number(page.planId)||0}})});const result=await response.json();if(!response.ok)throw new Error(result.error||'操作未完成，请重试。');note.textContent=result.message;if(button.dataset.platformAction==='save')location.assign('/plan?planId=${Number(page.planId)||0}&site=zhaopin&platformSaved=1');}catch(error){note.textContent=error.message;}finally{buttons.forEach(b=>b.disabled=false);}}));})();</script>` : ''}</div>`;
}
