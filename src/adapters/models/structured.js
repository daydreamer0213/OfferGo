...
class StructuredModelAdapter {
  ...
  async generateResumeOptimization(input) {
    const prompt = [
      ...
      "返回 JSON：{headline,suggestions:[{id,operation,originalText,proposedText,reason,evidenceIds,editingPrinciple}]}。suggestions 必须为 1-12 条（最多 12 条）；operation 只能是 replace、remove、insert_after。",
...
StructuredModelAdapter.prototype.draftMessageGroup = async function draftMessageGroup(input = {}, { signal = null } = {}) {
  ...
  const prompt = [
    ...
    "除 interview_invitation、manual_review、salary、sensitive、identity_uncertain 或确实缺少候选人事实外，必须返回 1-2 条草稿。",
