...
(async () => {
  ...
  assert(planHtml.includes("扩大范围时的搜索规模"));
  assert(planHtml.includes("最多读取多少份岗位详情"));
  assert(planHtml.includes("按日常范围找岗"));
  assert(planHtml.includes("扩大范围找岗"));
  assert(planHtml.includes('name="scanKind" value="daily"'));
  assert(planHtml.includes('name="scanKind" value="broad"'));
  assert(planHtml.includes("补全岗位详情"));
  assert(planHtml.includes("更新岗位状态"));
  assert(planHtml.includes("访问间隔和安全限制仍由 OfferGo 自动控制"));
  ...
  assert(activePlanHtml.includes('value="daily">按日常范围找岗</button>'), "仅有待确认草稿时，方案页扫描按钮不得被禁用");
